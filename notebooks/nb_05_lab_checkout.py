...
def _(mo):
    mo.accordion(
        {
            "💡 Hint 1 (a nudge)": "Two bugs, both logic (nothing crashes). First: the salad is part of the order, so it should be *added*, not subtracted. Second: the receipt's f-string is formatting `_old_total` (a stale leftover) — it should format the real total instead.",
...
def _(answer_ex50, mo):
    if answer_ex50 == "":
        ...
        ex50_ok = False
        _msg = (
            "❌ Quiz: not quite. Catching an error means the program **recovers** — "
            "it doesn't crash, and it doesn't loop back. After the `except` runs, "
            "execution simply continues past the `try` / `except`."
        )
    mo.md(_msg)
    return (ex50_ok,)
