...
``` python
print(check["total_eur"].mean())
```

a\) `20.0` b) an `AttributeError` c) `14.5`
...
print(check[check["zone"] == "Nord"]["total_eur"].mean())    # Nord only
```
...
No traceback, a plausible number, and still the wrong answer. **"It runs" is step 2. Step 3 is where the investor's number gets checked.**
...
## Predict: what does a comparison return?
...
a\) a column of `True`/`False`, one per row b) only the rows whose total is over 20 c) a single `True`, since some order is over 20
...
print(df[big])          # the rows: build the mask, then apply it
```
