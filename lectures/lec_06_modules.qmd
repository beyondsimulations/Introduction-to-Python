...
print("This is hard with just .find() and .split()!")
```

. . .

Regular expressions solve these problems by finding **patterns**, not just exact text.

## Regular Expressions: Step by Step

Let's work with this sample text throughout our examples:
...
# Find usernames of any length
result = re.findall(r'User \w+', sample_text)
print(f"All usernames: {result}")
```
...
- With uv, the environment is created and managed automatically for the project — no manual venv creation or activation needed
- This becomes especially important when working on several projects at once
