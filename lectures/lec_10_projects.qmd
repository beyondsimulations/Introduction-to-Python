...
# [Episode 10: The Exit]{.flow} {.title}
...
# [Form your pair, pick your direction]{.flow} {.title}

## Take ~25 minutes

Right now, in the room:

- **Find your partner** (or decide to go solo if the count is odd)
...
:::{.callout-tip}
**A good outcome by the end of this block:** a **one-sentence pitch** ("we're building X that does Y for Z") and a **name** for the project. That's all — the sentence keeps you honest about scope; the name goes on the repo in a minute.
:::

# [Your Toolchain]{.flow} {.title}

## Check your install

Before we build anything, the two installs from the Session IX homework need to work. In a terminal:

```bash
uv --version
```

- A version number? You're set.
...
Before any push reaches GitHub, your machine has to prove it's allowed to. Signing in to **Zed does not do this** — git needs its own setup. Once per computer, in a terminal:
...
GitHub doesn't know about your project yet. Three steps:
...
One of you owns the repo and invites the other (GitHub → **Settings → Collaborators**). The **partner** gets onto the project by cloning it:
