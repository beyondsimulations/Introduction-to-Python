...
[>Task:]{.task} Open Zed and copy the following code to `main.py`.
...
## Debugging Tools
[>Task:]{.task} Run the code and use the debugging tools by clicking on the small bug icon in the lower right corner to find the error and select `run [YOUR PATH TO THE FILE]/main.py`.
...
# [Script Organization]{.flow} {.title}

## The Main Function Pattern

- You'll often see `if __name__ == "__main__":` at the end of Python scripts
- This checks [whether the script is being run directly]{.highlight} or imported
...
- **Reusability**: Functions can be imported without executing test code
- **Testing**: You can test functions in the same file
- **Organization**: Clear separation between definitions and execution
