...
# 📋 Checkpoint 2 — before the doors open {.exercise-slide}
...
# [Episode 5: The 3-AM Checkout]{.flow} {.title}
...
# [Reading the Explosion]{.flow} {.title}
...
A **`try` / `except`** block runs risky code and catches the failure instead of stopping the program:
...
try:
    tip = float("generous")   # this raises ValueError...
except ValueError:
...
![](assets/qr/ex_05_a.png){width=280}

First **predict** what happens — then run it.

# [Debugging and Defending]{.flow} {.title}
...
![](assets/qr/ex_05_b.png){width=280}
...
::: {.callout-note}
**Next time — Episode 6** opens Part I's finale. It starts with **Checkpoint 3**, which sweeps everything from Episodes 1–5 — so keep this notebook and the last four close. Then the authorities notice the startup is quietly holding a pile of **customer data**, and somewhere in a grey office a form is being readied. **Formular 27b/6 awaits.**
:::
