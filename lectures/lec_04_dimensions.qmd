...
The `stop` index is left out. `drinks[0:2]` gives you two items, not three.
...
Assigning to a key **updates** it if it exists, or **adds** it if it doesn't:
...
prices = {"Mate": 3.50, "Spezi": 3.20}
prices["Spezi"] = 3.40       # update an existing key
prices["Kombucha"] = 4.20    # add a brand-new key
print(prices)
```
