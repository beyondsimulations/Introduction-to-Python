...
Python checks branches **top to bottom** and stops at the **first** `True`. So order matters — a loose test placed too early swallows the rest:

```{python}
#| eval: true
#| output-location: fragment

past_orders = 25            # a Gold customer!
if past_orders >= 5:
    tier = "Silver"         # 25 stops HERE — wrong
elif past_orders >= 20:
...
Test the **strictest** condition first. (One misplaced `elif` and Kevin demotes every VIP.)
