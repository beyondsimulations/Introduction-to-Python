...
# 📋 Checkpoint 1 — the quarterly board review {.exercise-slide}
...
# [Episode 3: The Copy-Paste Soup]{.flow} {.title}
...
![](assets/qr/ex_03_a.png){width=280}

First **predict** what happens — then run it.

# [Scope, then a First Class]{.flow} {.title}
...
class Delivery:
    ...
    def fee(self):                       # a method — note self
        return round(self.distance_km * 1.20, 2)
```

. . .

`__init__` sets up a new object · `self` is *this* object · attributes are stored on `self` and read back through `self`.
...
![](assets/qr/ex_03_b.png){width=280}
