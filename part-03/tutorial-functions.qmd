...
```{python}
# TODO: Start by looking at your code from the previous tutorial and think about the different steps that you implemented. 
# a) Try to modularize each step of the game into a separate function. Name the functions appropriately, to make clear what they do.
# b) Enhance the game by adding difficulty levels as described in the bonus task from the previous tutorial. The difference here is that the difficulty level should be given as a parameter to a function. If no difficulty level is given, i.e. the game master just enters an empty input, the function should use a default value of "Medium".
# Your code here
